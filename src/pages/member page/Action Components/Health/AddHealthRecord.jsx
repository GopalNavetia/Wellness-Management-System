...
export default function AddHealthRecord() {
    ...
    let handleSubmit = async (e) => {
        ...
        try {
            const response = await axiosInstance.post(`/MyProject/AddHealthRecordAPI?id=${memberID}`, formData, {
                headers: {
            ...
            if (response.data.success) {
                alert("Add Health Record Successful.")
                setFormData({
                    medical_history: "", current_medication: "", allergy: "", surgery: "", injury: "", supplement: "", diet_preference: "", drink: "", smoke: ""
                });
                navigate(-1);
            } else {
                alert('Failed to add health record: ' + response.data.message);
            }
