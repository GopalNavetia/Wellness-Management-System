...
export default function AddWorkoutPlan() {
    ...
    let handleSubmit = async (e) => {
        ...
        try {
            const response = await axiosInstance.post(`/MyProject/NewWorkoutAPI?id=${memberID}`, formData, {
                headers: {
            ...
            if (response.data.success === true) {
                alert("Add Workout Plan Successful.");
                setFormData({ start_date: "", end_date: "", monday: "", tuesday: "", wednesday: "", thursday: "", friday: "", saturday: "", sunday: "" });
                navigate(-1);
            } else {
                alert('Failed to Add Workout Plan: ' + (response.data.message || 'Unknown error'));
            }
        } catch (error) {
